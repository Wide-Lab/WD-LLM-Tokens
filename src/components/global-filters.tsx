...
import { useApiConfig, apiFetch } from "@/lib/api-config";
...
export function GlobalFilters() {
  ...
  const aplicacoes = useQuery({
    queryKey: ["aplicacoes", config.baseUrl, config.apiKey],
    queryFn: () => apiFetch<string[]>(config, "/v1/aplicacoes"),
    enabled: isConfigured,
    staleTime: 60_000,
  });

  const modelos = useQuery({
    queryKey: ["modelos", config.baseUrl, config.apiKey],
    queryFn: () => apiFetch<string[]>(config, "/v1/modelos"),
    enabled: isConfigured,
    staleTime: 60_000,
  });
